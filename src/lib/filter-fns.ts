      ...
      case "dateRange": {
        ...
        const dateValue =
          rowValue instanceof Date
            ? rowValue
            : new Date(rowValue as string | number | Date);
        ...
        switch (operator) {
          ...
          case "lt": // Is before
            return filterDate
              ? isBefore(dateValue, startOfDay(filterDate))
              : true;
          case "lte": // Is on or before
            return filterDate
              ? isBefore(dateValue, endOfDay(filterDate))
              : true;
          case "gt": // Is after
            return filterDate ? isAfter(dateValue, endOfDay(filterDate)) : true;
          case "gte": // Is on or after
            return filterDate
              ? isAfter(dateValue, startOfDay(filterDate))
              : true;
          case "isBetween": {
            if (filterDate && filterDateEnd) {
              return (
                (isAfter(dateValue, startOfDay(filterDate)) ||
                  isSameDay(dateValue, filterDate)) &&
